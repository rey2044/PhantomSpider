...
"""
PhantomSpider v2.0 – JS-Aware BFS Web Crawler with Random User-Agent Support
"""
...
parser = argparse.ArgumentParser(
    description="🕷️ PhantomSpider – BFS Web Crawler v2.0.0",
    epilog="""
Examples:
  python PhantomSpider.py -u https://example.com
  python PhantomSpider.py -u https://example.com --save -t 3
GitHub: https://github.com/rey2044 | Author: Laykumar Patel
""",
...
def banner():
    ...
    console.print("[bold yellow] 🕷️ [/] [bold bright_magenta] PhantomSpider v2.0 [/] [bold yellow] 🕷️ [/]", justify="center")
    console.rule("[bold yellow] 🕷️ [bold cyan] PhantomSpider – BFS Web Crawler [bold yellow] 🕷️", style="bright_magenta")
...
def show_table():
    ...
    console.print(table)
    console.rule("[bold yellow]🕷️[bold cyan] PhantomSpider", style="bright_magenta")

def save_results():
    all_urls = visited.union(found)
    urls_file = results_dir / "urls_discovered.txt"
    params_file = results_dir / "parameters_found.txt"
    dedup_file = results_dir / "deduped_params.txt"
    ...
    console.print(f"[green]✔ Results saved in:[/] {results_dir}")
